...
const existing = {
  ...
  extension: 'cs',
  size: 3,
  storageKey: 'users/u1/f1',
  createdAt: new Date('2026-09-01T10:00:00Z'),
  updatedAt: new Date('2026-09-01T10:00:00Z'),
  uploadedById: 'u1',
  modifiedById: 'u1',
  uploadedBy: { username: 'bohdan' },
  modifiedBy: { username: 'bohdan' },
};
...
describe('FilesService.upsert', () => {
  ...
  it('creates a new entry with uploadedBy = modifiedBy = caller', async () => {
    ...
    prisma.fileEntry.create.mockImplementation(
      async ({ data }: { data: Record<string, unknown> }) => ({
        ...existing,
        ...data,
        id: 'f9',
        storageKey: 'users/u1/f9',
        uploadedBy: { username: 'bohdan' },
        modifiedBy: { username: 'bohdan' },
      }),
    );
    prisma.fileEntry.update.mockImplementation(
      async ({ data }: { data: Record<string, unknown> }) => ({
        ...existing,
        ...data,
        id: 'f9',
  ...
  it('overwrites an existing name: same id and key, new bytes, modifiedBy = caller', async () => {
    ...
    expect(prisma.fileEntry.update.mock.calls[0][0].data).toMatchObject({
      size: 4,
      modifiedById: 'u2',
    });
  ...
  it('rejects a name with path separators', async () => {
    const badUpload = {
      originalname: '../evil.cs',
      buffer: Buffer.from('bad'),
      mimetype: 'text/plain',
      size: 3,
    };
    await expect(service.upsert(owner, badUpload)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  ...
  it('rejects an empty name', async () => {
    ...
    await expect(service.upsert(owner, emptyUpload)).rejects.toBeInstanceOf(
      BadRequestException,
    );
    expect(prisma.fileEntry.findUnique).not.toHaveBeenCalled();
    expect(prisma.fileEntry.create).not.toHaveBeenCalled();
  ...
  it('rejects a name with a backslash', async () => {
    const backslashUpload = {
      originalname: 'a\\b.cs',
      buffer: Buffer.from('bad'),
      mimetype: 'text/plain',
      size: 3,
    };
    await expect(service.upsert(owner, backslashUpload)).rejects.toBeInstanceOf(
      BadRequestException,
    );
    expect(prisma.fileEntry.findUnique).not.toHaveBeenCalled();
    expect(prisma.fileEntry.create).not.toHaveBeenCalled();
  ...
  it('refuses to read a file from another space', async () => {
    prisma.fileEntry.findUnique.mockResolvedValue({
      ...existing,
      ownerId: 'someone-else',
    });
    await expect(service.getContent('u1', 'f1')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('removes the created row when the upload to storage fails', async () => {
    prisma.fileEntry.findUnique.mockResolvedValue(null);
    prisma.fileEntry.create.mockResolvedValue({
      ...existing,
      id: 'f9',
      storageKey: '',
    });
    ...
    expect(prisma.fileEntry.delete).toHaveBeenCalledWith({
      where: { id: 'f9' },
    });
