...
import { extensionOf, isSafeFileName } from '@minidrive/shared';
...
export class FilesService {
  ...
  async upsert(owner: JwtUser, file: Upload): Promise<FileDto> {
    ...
    try {
      ...
      const saved = await this.prisma.fileEntry.update({
        where: { id: created.id },
        data: { storageKey },
        include: withUsers,
      });
  ...
  async delete(ownerId: string, id: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `Failed to delete storage object ${entry.storageKey}: ${(err as Error).message}`,
      );
    }
  }
