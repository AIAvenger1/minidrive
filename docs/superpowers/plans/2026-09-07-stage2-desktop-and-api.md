...
## Part D — UI improvement, tests, packaging, report

### Task 19: UI improvement — Tailwind CSS and shadcn/ui in `packages/ui`

**Files:**
- Create: `packages/ui/components.json`, `packages/ui/src/lib/utils.ts`, `packages/ui/src/components/ui/*.tsx` (generated by the shadcn CLI: `button`, `input`, `label`, `table`, `select`, `checkbox`, `card`, `badge`, `dialog`, `progress`, `alert`), `packages/ui/src/theme.css`
- Modify: `packages/ui/package.json`, `packages/ui/tsconfig.json`, `packages/ui/src/styles.css`, `packages/ui/src/FileTable.tsx`, `SortControl.tsx`, `FilterControl.tsx`, `ColumnToggle.tsx`, `UploadDropzone.tsx`, `PreviewPanel.tsx`, `apps/desktop/electron.vite.config.ts`, `apps/desktop/src/renderer/src/main.tsx`, `apps/desktop/src/renderer/src/styles.css`, `apps/desktop/src/renderer/src/screens/LoginScreen.tsx`, `apps/desktop/src/renderer/src/screens/DriveScreen.tsx`, `apps/desktop/src/renderer/src/components/SyncPanel.tsx`

**Interfaces:**
- Consumes: every component prop type from Tasks 14, 15 and 18 — **props do not change**; only the rendering does.
- Produces: `packages/ui/src/theme.css` (Tailwind v4 entry: `@import "tailwindcss"; @source` directives for `packages/ui/src` and both apps' renderer/src trees; shadcn CSS variables for light and dark); `cn()` in `packages/ui/src/lib/utils.ts`; shadcn primitives under `packages/ui/src/components/ui/` re-exported from `packages/ui/src/index.ts` (`Button`, `Input`, `Label`, `Table*`, `Select*`, `Checkbox`, `Card*`, `Badge`, `Dialog*`, `Progress`, `Alert*`). Next.js (Stage 3) will import the same `theme.css` and set `transpilePackages: ['@minidrive/shared', '@minidrive/ui']`.

- [ ] **Step 1: Install Tailwind v4 and shadcn prerequisites**

```bash
yarn workspace @minidrive/ui add tailwindcss @tailwindcss/vite class-variance-authority clsx tailwind-merge lucide-react radix-ui
yarn workspace @minidrive/ui add -D @types/react-dom
yarn workspace @minidrive/desktop add @tailwindcss/vite
```
`packages/ui/tsconfig.json` gains `"baseUrl"`-free path aliases: `"paths": { "@/*": ["./src/*"] }` (the shadcn CLI writes imports as `@/lib/utils` and `@/components/ui/...`). Add the same alias to `apps/desktop/tsconfig.web.json` `paths` pointing at `../../packages/ui/src/*` so the desktop typecheck resolves the generated imports, and to `electron.vite.config.ts` renderer `resolve.alias` (`'@': resolve('../../packages/ui/src')`).

- [ ] **Step 2: Initialise shadcn in `packages/ui`**

From `packages/ui`: `npx shadcn@latest init --yes --base-color neutral --css src/theme.css` (choose "Vite" style if prompted; the CLI writes `components.json`, `src/lib/utils.ts` with `cn`, and the theme CSS with `@import "tailwindcss"` plus the `:root` / `.dark` variable blocks). Then:
```bash
npx shadcn@latest add button input label table select checkbox card badge dialog progress alert
```
If the CLI refuses a monorepo layout, create `components.json` by hand:
```json
{
...
```
and re-run the `add` command. Append to `src/theme.css` after the import line:
```css
@source "../src";
@source "../../../apps/desktop/src/renderer/src";
@source "../../../apps/web/src";
```
Re-export the generated primitives from `packages/ui/src/index.ts` (`export * from './components/ui/button';` etc.). Delete the hand-written rules in `packages/ui/src/styles.css` that the components replace (keep only rules no primitive covers, e.g. `.drop-hint`), and import `theme.css` from it: `@import './theme.css';`.

- [ ] **Step 3: Wire Tailwind into the desktop renderer**

`apps/desktop/electron.vite.config.ts` renderer section: `plugins: [react(), tailwindcss()]` with `import tailwindcss from '@tailwindcss/vite'`. `main.tsx` imports `@minidrive/ui/src/styles.css` (already) — the desktop `styles.css` keeps only `.login`, `.drive` and `.sync` layout rules, rewritten with Tailwind utility classes in the components where simpler.

- [ ] **Step 4: Re-skin the shared components (props unchanged)**

- `FileTable`: `Table`, `TableHeader`, `TableRow`, `TableHead`, `TableBody`, `TableCell`; selected row gets `data-state="selected"`; sizes and dates formatted as before; empty state row uses `text-muted-foreground`.
- `SortControl`: `Button variant="outline" size="sm"` with `ArrowUpAZ` / `ArrowDownAZ` icons from `lucide-react` and the label «Назва».
- `FilterControl`: `Select` with the two options «Усі файли» / «Лише .cpp, .png».
- `ColumnToggle`: a row of `Checkbox` + `Label` pairs, `name` never listed.
- `UploadDropzone`: `Button` «Завантажити файли» with an `Upload` icon; the drop overlay uses `border-2 border-dashed border-primary bg-primary/5`.
- `PreviewPanel`: `Card` with `CardHeader` (file name + `Badge` with the extension), `CardContent` with a `dl` for the attributes, `<pre className="font-mono text-xs">` for text and `<img className="max-w-full h-auto rounded">` for images; `Alert variant="destructive"` for the load error.

- [ ] **Step 5: Re-skin the desktop screens**

- `LoginScreen`: `Card` centered with `CardHeader` («MiniDrive»), three `Label` + `Input` pairs (`autoComplete="username"` / `"current-password"`, `required`), `Alert variant="destructive"` for errors with `aria-live="polite"`, `Button` submit, `Button variant="link"` to switch mode; clear `error` when switching mode.
- `DriveScreen`: header with the app name, username as a `Badge`, «Вийти» `Button variant="ghost"`; toolbar in a `flex flex-wrap gap-3`; delete uses `Dialog` («Видалити файл?», «Скасувати» / «Видалити» `variant="destructive"`) instead of `confirm()`.
- `SyncPanel`: `Card` with the folder path in `font-mono text-xs truncate`, `Progress` for the transfer, `Alert` for the report/errors, `Checkbox` for auto-watch.

- [ ] **Step 6: Verify and commit**

Run: `yarn workspace @minidrive/desktop typecheck`, `yarn workspace @minidrive/shared test`, `yarn workspace @minidrive/desktop test`, then `env -u ELECTRON_RUN_AS_NODE yarn workspace @minidrive/desktop dev` with the API stack up: log in, upload, sort, filter, hide columns, preview `.cs` and `.jpg`, delete via the dialog, sync. Every feature from Tasks 13–18 must still work; take no screenshots yet (Task 20 does).

```bash
git add packages/ui apps/desktop yarn.lock
git commit -m "feat(ui): tailwind and shadcn components" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"
```

---

### Task 20: Test run, packaging, screenshots and the Stage 2 report
