...
![Панель синхронізації після натискання «Синхронізувати»: рядок звіту «Завантажено 0, вивантажено 6, пропущено 0, помилок 0» — локальних файлів для відправки не було, усі 6 файлів скачано на щойно прив'язану порожню локальну папку](img/09-sync.png){width=15cm}

## Правила синхронізації

Реалізовано в `computeSyncPlan` (`packages/shared/src/sync.ts`) і покрито тестами `sync.test.ts` та `syncEngine.test.ts`: файл лише локально → вивантажується; файл лише віддалено → скачується; за наявності з обох боків з однаковим розміром і `|mtime − updatedAt| ≤ 2000` мс — пропускається; інакше перемагає новіша версія (за `mtime`/`updatedAt`); видалення ніколи не поширюються в інший бік (видалення локально відновлює файл з сервера і навпаки); після скачування локальний `mtime` виставляється рівним `updatedAt` сервера, щоб наступна синхронізація не вважала файл зміненим.

# Unit-тестування

| Файл тестів | Що перевіряє | К-сть |
|---|---|---|
| `packages/shared/src/fileList.test.ts` | `sortByName` (зростання/спадання, стабільність, регістронезалежність — **операція варіанта**), `filterByType` (усі файли / лише `.cpp`,`.png` — **операція варіанта**, окремо копія масиву для `'all'`), `extensionOf` (звичайне розширення й порожнє для імені, що закінчується голою крапкою), `isSafeFileName`, `isSyncableName` | 12 |
