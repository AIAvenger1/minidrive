...
subtitle: "Веб-орієнтована версія (Next.js) та розгортання"
author:
...
`FilterControl` — той самий випадаючий список «Усі файли» / «Лише `.cpp`, `.png`».

![Фільтр «Лише .cpp, .png»: залишились logo.png і main.cpp](img/web-04-filter.png){width=15cm}
...
|---|---|---|
| `apps/web/src/lib/browserSync.test.ts` | `BrowserSyncEngine.synchronize` на пам'ятевій фейковій директорії: одночасне надсилання локального-лише файлу й скачування віддаленого-лише файлу з фіксацією обох у журналі; пропуск файлу, який журнал уже знає, попри неможливість браузера встановити mtime; підрахунок помилок без переривання решти передавань і звіт про прогрес (`onProgress`) | 4 |
| `packages/shared/src/sync.test.ts` (нові випадки) | `runSyncPlan` — спільний цикл виконання плану для обох клієнтів: підрахунок надісланих/скачаних/пропущених файлів, фіксація помилки без переривання решти передавань, повідомлення про прогрес, зникнення віддаленого запису як помилка | 4 |
| `packages/shared/src/syncLedger.test.ts` | `reconcileWithLedger` (підміна `mtime` на серверний за збігом розміру й mtime, залишення «сирого» файлу без змін інакше), `recordTransfer`, `pruneLedger` (видалення записів про зниклі файли) | 6 |
